class OutboundRidesToYellowsController < ApplicationController
  ...
  def total_rides_per_location
    ...
    pre_result[l] = Rides.where("location = ? AND service = ?", l, 'green').map{|r| r.outbound_to_yellow}.inject(:+)
    }
  ...
  def total_fare_per_ride
    ...
    locations_parsed.each{|l|
    pre_totalrides[l] = Rides.where("location = ? AND service = ?", l, 'green').map{|r| r.outbound_to_yellow + r.outbound_to_green + r.outbound_to_ewr + r.outbound_to_airports + r.outbound_to_unknown}.inject(:+)
    }
  ...
  def bubble_chart_data
    ...
    locations_parsed = locations.map{|l| l.location}.select{|k| k != 'Rikers Island' && k != 'NV'}.select{|k| k != 'Rikers Island' && k != 'NV' && k != "Eltingville/Annadale/Prince's Bay" && k != "unknown zone"}.sort
    ...
    locations_parsed.each{|l|
    totalrides[l] = Rides.where("location = ? AND service = ?", l, 'green').map{|r| r.outbound_to_yellow}.inject(:+)
    }

    locations_parsed.each{|l|
    totalridestoall[l] = Rides.where("location = ? AND service = ?", l, 'green').map{|r| r.outbound_to_green + r.outbound_to_yellow + r.outbound_to_ewr + r.outbound_to_airports + r.outbound_to_unknown}.inject(:+)
    }

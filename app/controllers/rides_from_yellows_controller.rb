class RidesFromYellowsController < ApplicationController



  def locations
    locations = Rides.select(:location).distinct
    result = locations.map{|l| l.location}.select{|k| k != 'Rikers Island' && k != 'NV'}
    render json: result
  ...
  def total_rides_per_location
    ...
    locations_parsed = locations.map{|l| l.location}.select{|k| k != 'Rikers Island' && k != 'NV'}

    locations_parsed.each{|l|
    pre_result[l] = Rides.where("location = ? AND service = ?", l, 'green').map{|r| r.inbound_from_yellow}.inject(:+)
    }
  ...
  def total_distance_per_location
    ...
    locations_parsed.each{|l|
    pre_result[l] = Rides.where("location = ? AND service = ?", l, 'green').map{|r| r.inbound_sum_distance}.inject(:+)
    }
  ...
  def total_fare_per_location
    ...
    locations_parsed.each{|l|
    pre_result[l] = Rides.where("location = ? AND service = ?", l, 'green').map{|r| r.inbound_sum_fare}.inject(:+)
    }
  ...
  def total_fare_per_mile
    ...
    locations_parsed.each{|l|
    pre_totalfares[l] = Rides.where("location = ? AND service = ?", l, 'green').map{|r| r.inbound_sum_fare}.inject(:+)
    }

    locations_parsed.each{|l|
    pre_totalmiles[l] = Rides.where("location = ? AND service = ?", l, 'green').map{|r| r.inbound_sum_distance}.inject(:+)
    }
  ...
  def total_fare_per_ride
    ...
    locations_parsed.each{|l|
    pre_totalrides[l] = Rides.where("location = ? AND service = ?", l, 'green').map{|r| r.inbound_from_yellow + r.inbound_from_green + r.inbound_from_airports + r.inbound_from_unknown}.inject(:+)
    }
  ...
  def bubble_chart_data
    ...
    locations_parsed = locations.map{|l| l.location}.select{|k| k != 'Rikers Island' && k != 'NV'}.select{|k| k != 'Rikers Island' && k != 'NV'}.sort

    locations_parsed.each{|l|
    totalmiles[l] = Rides.where("location = ? AND service = ?", l, 'green').map{|r| r.inbound_sum_distance}.inject(:+)
    }

    locations_parsed.each{|l|
    totalfares[l] = Rides.where("location = ? AND service = ?", l, 'green').map{|r| r.inbound_sum_fare}.inject(:+)
    }

    locations_parsed.each{|l|
    totalrides[l] = Rides.where("location = ? AND service = ?", l, 'green').map{|r| r.inbound_from_yellow}.inject(:+)
    }

    locations_parsed.each{|l|
    totalridesfromall[l] = Rides.where("location = ? AND service = ?", l, 'green').map{|r| r.inbound_from_green + r.inbound_from_yellow + r.inbound_from_airports + r.inbound_from_unknown}.inject(:+)
    }
    ...
    totalfares.each{|k,v|
        ...
        result[k]['totalridesfromall'] = totalridesfromall[k]
...
#
# def total_rides_per_location_yellow
#   result = {}
#   pre_result = {}
#
#   locations = Rides.select(:location).distinct

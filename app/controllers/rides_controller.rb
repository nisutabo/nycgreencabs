class RidesController < ApplicationController
  ...
  def locations
    ...
    result = locations.map{|l| l.location}.select{|k| k != 'Rikers Island' && k != 'NV'}.sort
    render json: result
  ...
  def total_rides_per_location
    ...
    pre_result[l] = Rides.where("location = ? AND service = ?", l, 'green').map{|r| r.inbound_from_green}.inject(:+)
    }
  ...
  def total_fare_per_ride
    ...
    locations_parsed.each{|l|
    pre_totalrides[l] = Rides.where("location = ? AND service = ?", l, 'green').map{|r| r.inbound_from_green + r.inbound_from_yellow + r.inbound_from_airports + r.inbound_from_unknown}.inject(:+)
    }
  ...
  def bubble_chart_data
    ...
    locations_parsed.each{|l|
    totalrides[l] = Rides.where("location = ? AND service = ?", l, 'green').map{|r| r.inbound_from_green}.inject(:+)
    }
